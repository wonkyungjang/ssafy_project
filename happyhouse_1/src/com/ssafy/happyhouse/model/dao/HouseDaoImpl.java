...
public class HouseDaoImpl implements HouseDao{
	...
	public HouseDeal search(int no) {
		...
		for(HouseDeal h:search) {
			if(h.getNo()==no) {
				String s=h.getDong()+h.getAptName(); //얘의 벨류를 받아옴
				if(houseInfo.containsKey(s)) {
					h.setImg(houseInfo.get(s).getImg());
					h.setBuildYear(houseInfo.get(s).getBuildYear());
					h.setJibun(houseInfo.get(s).getJibun());
					h.setCode(houseInfo.get(s).getCode());
					System.out.println();
					return h;
				}
			}
